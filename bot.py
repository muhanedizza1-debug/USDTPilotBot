...
<section id="deposit" class="page">
    <div class="card" style="background: var(--card-bg); border: 1px solid var(--card-border); border-radius: 20px; padding: 20px;">
        ...
        <select id="deposit-network" onchange="updateDepositAddress()">
            ...
            <option value="TON">TON Network (USDT-TON)</option>
        </select>
...
<script>
    ...
    const cryptoNetworks = {
        ...
        'TON': { name: 'TON Network', tag: 'Tether USD (TON)', address: 'EQA000000000000000000000000000000000000000000000', icon: 'https://cryptologos.cc/logos/toncoin-ton-logo.png' }
    };
        ...
        setInterval(() => {
            ...
            userState.investments.forEach((inv, index) => {
                ...
                if (elapsedHours < (inv.durationDays * 24)) {
                    ...
                    showToast(`Plan $${inv.amount} completed! Capital unlocked.`);
                }
            });
